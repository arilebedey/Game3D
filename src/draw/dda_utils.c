...
static void	set_text_pt_x(t_ctx *ctx, t_dda *ref_dda, int i, double perp_wall_dist);
...
void	init_draw_elem(t_ctx *ctx, t_dda *ref_dda, int i)
{
	...
	if (ref_dda->side == 0)
		perp_wall_dist = (ref_dda->side_dist.x - ref_dda->delta_dist.x);
	else
		perp_wall_dist = (ref_dda->side_dist.y - ref_dda->delta_dist.y);
...
static void	set_text_pt_x(t_ctx *ctx, t_dda *ref_dda, int i, double perp_wall_dist)
{
	...
	*ref_tex_x = (int)(wall_x * (double)ref_dda->col_draw_arr[i].texture.width);
	if ((ref_dda->side == 0 && ref_dda->ray.x < 0) || (ref_dda->side == 1 && ref_dda->ray.y > 0))
